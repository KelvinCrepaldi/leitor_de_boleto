...
import {
  ...
  generateBarCode,
  generateDV,
} from "../services/boleto/getBoletoService";
...
it("Generate CV code for BarCode.", () => {
  const result = generateDV("21299758700000020000001121100012100447561740");
  expect(result).toBe(9);
});

it("Generate CV code for BarCode.But if it generate a code equal 0, it returns 1", () => {
...
it("create object with barCode, amount and expirationDate generated correctly", () => {
  ...
  const result = new GetBoletoService().execute(
    "21290001192110001210904475617405975870000002000"
  );
